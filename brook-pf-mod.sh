...
Download_brook(){
    ...
    if [[ ${bit} == "x86_64" ]]; then
        ...
        wget --no-check-certificate -N "https://cdn.jsdelivr.net/gh/kellycrenshaw0019/brook-/brook"
    else		
		echo -e "${Info} 默认安装v20200801版"
        wget --no-check-certificate -N "https://cdn.jsdelivr.net/gh/kellycrenshaw0019/brook-/brook_linux_386"
...
Service_brook(){
    if [[ ${release} = "centos" ]]; then
        if ! wget --no-check-certificate https://cdn.jsdelivr.net/gh/kellycrenshaw0019/brook-/brook-pf_centos -O /etc/init.d/brook-pf; then
        ...
        if ! wget --no-check-certificate https://cdn.jsdelivr.net/gh/kellycrenshaw0019/brook-/brook-pf_debian -O /etc/init.d/brook-pf; then
            echo -e "${Error} Brook服务 管理脚本下载失败 !" && exit 1
        fi
        chmod +x /etc/init.d/brook-pf
...
if [[ "${action}" == "monitor" ]]; then
 ...
 ${Green_font_prefix} 0.${Font_color_suffix} 升级脚本（不可用）
————————————
 ${Green_font_prefix} 1.${Font_color_suffix} 安装 Brook
 ${Green_font_prefix} 2.${Font_color_suffix} 更新 Brook（不可用）
